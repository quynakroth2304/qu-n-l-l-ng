...
# ==============================================================================
# 1. CẤU HÌNH HỆ THỐNG V45 (PERFECT ALL-IN-ONE)
# ==============================================================================
st.set_page_config(
    page_title="Hệ Thống V45 Final", 
    layout="wide", 
    page_icon="💎", 
...
DATABASE_FILE = "system_v45_full.db"
...
# --- CSS GIAO DIỆN (FIX KHUNG CHAT & PAYMENT CARD) ---
st.markdown("""
<style>
    ...
    .bubble-right {
        ...
        text-align: left; 
        overflow-wrap: anywhere; /* Fix vỡ khung tuyệt đối */
...
def render_chat_window(room_id, current_user_name, current_role, chat_mode="group"):
    ...
    for msg_id, sender, content, timestamp, msg_type in messages:
        ...
        with st.container():
            ...
            if not is_me:
                with col1:
                    if sender != last_sender: st.image(get_avatar_url(sender), width=35)
            ...
            with target_col:
                ...
                if msg_type == 'payment_request':
                    amount = content
                    st.markdown(f"""<div class="payment-bubble" style="margin-bottom: 5px; align-self: {alignment};"><div class="payment-header">💸 YÊU CẦU XÁC NHẬN</div><div>Quản lý {sender} đã chuyển khoản lương:</div><div class="payment-amount">{int(float(amount)):,.0f} VNĐ</div></div>""", unsafe_allow_html=True)
                    ...
                    if current_role == 'staff' and not is_me:
                        ...
                        if pending:
                            if st.button("✅ BẤM ĐỂ XÁC NHẬN ĐÃ NHẬN TIỀN", key=f"pay_btn_{msg_id}", use_container_width=False):
                                ...
                                cursor.execute("INSERT INTO messages VALUES (NULL,?,?,?,?,?)", (room_id, current_user_name, f"✅ Đã xác nhận nhận đủ: {int(float(amount)):,.0f} VNĐ", datetime.now().strftime("%H:%M"), "text"))
                                connection.commit()
                                st.rerun()
                        else: st.caption("✅ Giao dịch đã hoàn tất")
                    else: st.caption("⏳ Đang chờ xác nhận...")

                elif msg_type == 'image':
                    if os.path.exists(content): st.image(content, width=250)
                    else: st.error("Ảnh đã xóa")
                elif msg_type == 'call':
                    link = content.split('|')[-1]
                    st.markdown(f"""<div style="background:#e0f2fe; padding:10px; border-radius:10px; width:fit-content; align-self:{alignment};">📹 <b>{sender}</b> đang gọi... <br><a href="{link}" target="_blank" style="font-weight:bold;">Tham gia ngay</a></div>""", unsafe_allow_html=True)
                else:
...
def render_dashboard(staff_list):
    ...
    for staff in staff_list:
        file_path = os.path.join(STORAGE_DIRECTORY, staff[0], "salary.xlsx"); df = load_excel_safe(file_path)
        ...
        if "Xác nhận đến" in df.columns: pending_approval += len(df[df["Xác nhận đến"].astype(str).str.lower() == "false"])
...
# ==============================================================================
# 4. AUTHENTICATION (BẢO MẬT KEY & ROLE)
# ==============================================================================
    ...
    with c2:
        ...
        with t1:
            ...
            if st.button("Login", type="primary", use_container_width=True):
                cursor.execute('SELECT * FROM users WHERE username=? AND password=?', (u, p)); ud = cursor.fetchone()
                if ud:
            ...
            if st.button("Đăng Ký", use_container_width=True):
                if not ru or not rpass: st.warning("Điền đủ!")
                    ...
                    try:
                        ...
                        cursor.execute('INSERT INTO users VALUES (?,?,?,?,?,?,?,?,?)', (ru, rpass, 'admin' if rr=="Quản lý" else 'staff', None, rn, rwp, rp, None, "2099-01-01")); connection.commit(); st.success("OK! Đăng nhập ngay."); 
        ...
        with c2:
            with st.popover("➕", use_container_width=True):
                ...
                if up and st.button("Gửi Ảnh", use_container_width=True):
                    e = up.name.split('.')[-1]; f = f"{uuid.uuid4()}.{e}"; p = os.path.join(UPLOAD_DIRECTORY, f)
...
with tw:
    if cr == 'admin':
        with st.expander("🏢 QUẢN LÝ CHI NHÁNH"):
            ...
            my_branches = pd.read_sql_query(f"SELECT * FROM workplaces WHERE created_by='{cu}'", connection)
        ...
        if sl:
            ...
            with c3:
                ...
                if cdebt > 0 and st.button("💸 BÁO ĐÃ CHUYỂN KHOẢN", use_container_width=True):
                    ...
                    dfs.loc[mask, "Trạng thái"] = "chờ xác nhận"; save_excel_safe(dfs, tf)
                    twp = [s[2] for s in sl if s[0] == tid][0]
                    cursor.execute("INSERT INTO messages VALUES (NULL,?,?,?,?,?)", (twp, cz, str(cdebt), datetime.now().strftime("%H:%M"), "payment_request")); connection.commit(); st.success("Đã gửi yêu cầu xác nhận!"); st.rerun()
            
            with st.expander("➕ Thêm Ca (Admin)"):
                with st.form("aa"):
        ...
        with st.expander("➕ Báo cáo ca", expanded=True):
            with st.form("sa"):
                d = st.date_input("Ngày"); v = st.text_input("VT", cwp); t1 = st.time_input("In"); t2 = st.time_input("Out"); sr = st.number_input("Lương/giờ", value=20000, step=1000)
